...
struct packed_vs_input_stencil_shadow
{
    float4 position [[attribute(0)]];
    float4 face_normal_0 [[attribute(1)]];
    float4 face_normal_1 [[attribute(2)]];
};
...
, packed_vs_input_stencil_shadow in_vertex [[stage_in]]
, constant c_per_pass_view &per_pass_view [[buffer(8)]]
, constant c_per_draw_call &per_draw_call [[buffer(9)]]
, constant c_cbuffer_single_light &cbuffer_single_light [[buffer(18)]])
{
    vs_input_stencil_shadow input;
    input.position = float4(in_vertex.position);
    input.face_normal_0 = float4(in_vertex.face_normal_0);
    input.face_normal_1 = float4(in_vertex.face_normal_1);
