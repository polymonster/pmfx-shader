...
, constant c_per_pass_vs &per_pass_vs [[buffer(9)]]
, constant c_image_ex &image_ex [[buffer(15)]])
{
    vs_input input;
    input.position = float2(in_vertex.position);
    input.tex_coord = float2(in_vertex.tex_coord);
    input.colour = float4(in_vertex.colour) * 0.00392156862;constant float4x4& projection_matrix = per_pass_vs.projection_matrix;
