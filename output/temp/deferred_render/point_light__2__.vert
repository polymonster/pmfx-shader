...
//deferred_render point_light__2__ vs 2
#ifdef GLES
...
layout(location = 1) out float4 screen_coord_vs_output;
layout(location = 2) out float4 light_pos_radius_vs_output;
layout(location = 3) out float4 light_dir_cutoff_vs_output;
layout(location = 4) out float4 light_colour_vs_output;
layout(location = 5) out float4 light_data_vs_output;
struct vs_input
{
    float4 position;
    float4 normal;
